...
function slideShow(speed) {
  ...
  $('ul.images li:first').css({opacity: 0.9}).addClass('show');
...
function gallery() {
  ...
  if(current.queue('fx').length == 0) {
    ...
    next.css({opacity: 0.0}).addClass('show').animate({opacity: 0.9}, 1300);
    
    // hide the current image
    current.animate({opacity: 0.0}, 1300).removeClass('show');
